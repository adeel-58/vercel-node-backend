...
router.post(
  ...
  async (req, res) => {
    const connection = pool; // Using your MySQL connection pool
    ...
    try {
      ...
      const storeId = supplier.id;

      // 2️⃣ Check if supplier has a plan
      if (!supplier.plan_id)
        return res.status(403).json({ success: false, message: "You do not have an active plan. Please subscribe first." });
      ...
      if (planRows.length === 0)
        return res.status(403).json({ success: false, message: "Your current plan is inactive or invalid." });

      const plan = planRows[0];

      // 4️⃣ Count products already uploaded by supplier
      const [productCountRows] = await connection.query(
        `SELECT COUNT(*) AS total FROM Product WHERE store_id = ?`,
        [storeId]
      );

      const productCount = productCountRows[0].total;

      // 5️⃣ Compare with upload limit
      if (productCount >= plan.upload_limit) {
        return res.status(403).json({
          success: false,
          message: `Upload limit reached for your current plan (${plan.name}). Please upgrade your plan to add more products.`,
        });
      }

      // 6️⃣ Continue adding product if allowed
      const {
        title,
        ebay_link,
      ...
      await connection.query(
        `UPDATE SupplierProfile SET total_products = total_products + 1 WHERE id = ?`,
        [storeId]
      );
